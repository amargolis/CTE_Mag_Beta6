...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        memCache = [NSMutableDictionary new];
        ...
        cacheInQueue = [NSOperationQueue new];
...
- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    if (!image && fromDisk)
    {
        image = [[UIImage new] initWithData:[NSData dataWithContentsOfFile:[self cachePathForKey:key]]];
        if (image != nil)
        {
            [memCache setObject:image forKey:key];
                    }
